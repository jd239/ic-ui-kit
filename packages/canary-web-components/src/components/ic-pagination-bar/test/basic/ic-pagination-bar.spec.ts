...
describe("ic-pagination-bar", () => {
  ...
  it("should set items per page dropdown to 50 if totalItems is 50 and selectedItemsPerPage is 100", async () => {
    ...
    expect(selectBtn!.textContent).toBe("50"); // Duplicate 50 items per page option removes "All" and keeps { label: "50", value: "50" }
  });
  ...
  it("should emit 'isUserAction: true' when explicitly changing items per page select value", async () => {
    ...
    const itemsPerPageSelect = paginationBar!.shadowRoot!.querySelector(".items-per-page-input") as HTMLIcSelectElement;

    itemsPerPageSelect!.click();

    const options = Array.from(itemsPerPageSelect!.shadowRoot!.querySelectorAll(".option"));
